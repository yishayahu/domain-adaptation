...
import numpy as np
import sys
import trainer_third_exp as trainer


def main(dataroot = 'digits'):
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataroot', default=None, help='path to source dataset')
    ...
    source_train_root = os.path.join(dataroot, 'mnist/trainset')
    source_val_root = os.path.join(dataroot, 'mnist/testset')
    target_root = os.path.join(dataroot, 'svhn/trainset')
    ...
    source_valloader = torch.utils.data.DataLoader(source_val, batch_size=opt.batchSize, shuffle=False, num_workers=2,
                                                   drop_last=False)
    targetloader = torch.utils.data.DataLoader(target_train, batch_size=opt.batchSize, shuffle=True, num_workers=2,
                                               drop_last=True)
    ...
    # Training
    if opt.method == 'GTA':
        full_trainer = trainer.FullTrainer(opt=opt, num_classes=nclasses,mean= mean,std= std,source_train_ds= source_trainloader,source_test_ds= source_valloader,target_train_ds= targetloader)
        full_trainer.train()
    elif opt.method == 'sourceonly':
        source_trainer = trainer.SourceTrainer(opt=opt, num_classes=nclasses, source_train_ds=source_trainloader, source_test_ds=source_valloader)
