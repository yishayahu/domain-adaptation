...
import numpy as np
import models_third_exp as models
import utils
import os
import argparse
...
def main(dataroot='digits'):
    ...
    parser.add_argument('--checkpoint_dir', default='results_third_exp/models', help='folder to load model checkpoints from')
    parser.add_argument('--method', default='GTA', help='Method to evaluate| GTA, sourceonly')
    parser.add_argument('--model_best', type=int, default=0,
                        help='Flag to specify whether to use the best validation model or last checkpoint| 1-model best, 0-current checkpoint')
    ...
    netF = models.Mixer(opt)
    netC = models.Classifier(opt, nclasses)
